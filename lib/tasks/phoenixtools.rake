namespace :phoenixtools do
	...
	task :setup => :environment do 
		...
		html.login
		Nexus.config.update!(:setup_notice => "Fetching affiliation attributes")
		...
		Nexus.config.update!(:setup_notice => "Extrapolating jump links")
		JumpLink.extrapolate_until_no_more!
		Nexus.config.setup_known_stargates_and_wormholes!
		Nexus.config.update!(:setup_notice => "Generating paths")
		Path.generate!
		Nexus.config.update!(:setup_notice => "Setup complete", :setup_complete => true, :core_fetched_at => Time.now, :jump_map_fetched_at => Time.now)
	end
end
class Nexus < ActiveRecord::Base
	...
	def schedule_setup!
		update!(:setup_complete => false, :setup_notice => "Starting setup")
		LOG.info "Starting setup"
		setup!
	end

	def schedule_daily_update!(no_bases=false)
		update!(:updating_market => true)
		update!(:updating_items => true)
		update_items!
		unless no_bases
			update!(:updating_turns => true)
			update_turns!
		end
		update_market!
	end
	...
	def schedule_full_update!
		update!(:updating_jump_map => true)
		update_jump_map!
		update!(:updating_cbodies => true)
		update_cbodies!
		update!(:updating_turns => true)
		update_turns!
	...
	def setup_known_stargates_and_wormholes!
		# STARGATES
		update!(:setup_notice => "Adding known stargates")
		...
		# WORMHOLES
		update!(:setup_notice => "Adding known wormholes")
	...
	def setup!
		...
		html.login
		update!(:setup_notice => "Fetching affiliation attributes")
		...
		Periphery.all.each do |p| 
			html = Nexus.html_client
			html.login
			Nexus.config.update!(:setup_notice => "Fetching #{p} jump map")
			LOG.info "Fetching #{p} jump map"
			html.get_jump_map(p.id)
		end
		update!(:setup_notice => "Extrapolating jump links")
		LOG.info "Extrapolating jump links"
		JumpLink.extrapolate_until_no_more!
		setup_known_stargates_and_wormholes!
		update!(:setup_notice => "Generating paths")
		LOG.info "Generating paths"
		Path.generate!
		update!(:setup_notice => "Setup complete", :setup_complete => true, :core_fetched_at => Time.now, :jump_map_fetched_at => Time.now)
	...
	def update_market!
		update!(:update_notice => "Fetching market data")
		LOG.info "Fetching market data"
		MarketXml.fetch!
		update!(:update_notice => "Generating trade routes") 
		LOG.info "Generating trade routes"
		TradeRoute.generate!
		update!(:update_notice => "Trade routes generated", :updating_market => false, :market_fetched_at => Time.now)
		LOG.info "Trade routes generated"
	end

	def update_items!
		update!(:update_notice => "Fetching item data")
		LOG.info "Fetching item data"
		Item.fetch_missing!
		update!(:update_notice => "Item data fetched", :updating_items => false, :items_fetched_at => Time.now)
		LOG.info "Item data fetched"
	end

	def update_cbodies!
		update!(:update_notice => "Fetching celestial body data")
		LOG.info "Fetching celestial body data"
		StarSystem.fetch_cbodies!
		update!(:update_notice => "Celestial bodies mapped", :updating_cbodies => false, :cbodies_fetched_at => Time.now)
	...
	def update_jump_map!
		...
		Periphery.all.each do |p| 
			html = Nexus.html_client
			html.login
			Nexus.config.update!(:update_notice => "Fetching #{p} jump map")
			LOG.info "Fetching #{p} jump map"
			html.get_jump_map(p.id)
		end
		update!(:update_notice => "Extrapolating jump links") 
		LOG.info "Extrapolating jump links"
		JumpLink.extrapolate_until_no_more!
		update!(:update_notice => "Generating paths") 
		LOG.info "Generating paths"
		Path.generate!
		update!(:update_notice => "Jump map up to date", :updating_jump_map => false, :jump_map_fetched_at => Time.now)
	...
	def update_turns!
		...
		client.login 
		update!(:update_notice => "Fetching turns for bases")
		LOG.info "Fetching turns for bases"
		Position.bases.each{|p| p.base.fetch_turn!}
		update!(:update_notice => "Base turns fetched", :updating_turns => false, :turns_fetched_at => Time.now)
